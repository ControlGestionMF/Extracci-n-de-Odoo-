...
def extraer_pedidos_venta(uid, models, batch_size=100, max_records=5000):
    fields = [
        ...
        'amount_untaxed', 'partner_shipping_id', 'state', 'invoice_status',
        'pricelist_id', 'payment_term_id', 'client_order_ref', 'validity_date'
    ]

    fecha_desde_dt = datetime.now() - relativedelta(months=6)
    ...
    many2one_campos = ['partner_id', 'user_id', 'pricelist_id', 'payment_term_id', 'partner_shipping_id']
    ...
    df_final = df.rename(columns={
        ...
        'partner_shipping_id_id': 'id_direccion_entrega',
        'partner_shipping_id_name': 'direccion_entrega',
        'state': 'estado_pedido',
    ...
    columnas = [
        'id_pedido', 'nombre_pedido', 'fecha_pedido', 'cliente', 'id_cliente',
        'vendedor', 'id_vendedor', 'monto_total', 'base_imponible',
        'id_direccion_entrega', 'direccion_entrega',
        'estado_pedido', 'estado_factura',
...
def guardar_en_sql(df):
    try:
        engine = create_engine(f"mysql+pymysql://{sql_user}:{sql_password}@{sql_host}:{sql_port}/{sql_db}")
        df.to_sql(sql_table, con=engine, if_exists='replace', index=False)
        print(f"✅ Datos guardados en MySQL correctamente (tabla: {sql_table})")
    except SQLAlchemyError as e:
        print(f"❌ Error al guardar en MySQL: {str(e)}")
...
def main():
    try:
        ...
        print("Extrayendo pedidos de venta últimos 6 meses (máx 5,000)...")
